...
#' }
Boruta.default<-function(x,y,pValue=0.01,quantileThresh=1,mcAdj=TRUE,maxRuns=100,doTrace=0,holdHistory=TRUE,getImp=getImpRfZ,...){
  #Timer starts... now!
  timeStart<-Sys.time()
  ...
  ##Assigns hits
  assignHits<-function(hitReg,curImp,quantileThresh){
    curImp$imp>quantile(curImp$shaImp, quantileThresh)->hits
    if(doTrace>2){
  ...
  while(any(decReg=="Tentative") && (runs+1->runs)<maxRuns){
    ...
    hitReg<-assignHits(hitReg,curImp,quantileThresh)
