...
datasets_A <- c('cd_franzosa_2019','cirrhosis_qin_2014','crc_feng_2015','crc_s3_s4_yachida_2019','esrd_wang_2020','metacardis_1_8','metacardis_3_8','uc_franzosa_2019','uc_spain_nielsen_2014')
...
for(curr_dataset in datasets_A) {
...
for (fs in common_feats) {
  ...
  all_feats <- data.frame(table(tmp$feature)) %>% arrange(Freq) %>% pull(Var1) %>% as.character()
...
Only component pairs with at least 3 common features.

```{r eval=F}
module_intersections %>%
  filter(n_intersection >= 3) %>%
  select(-fisher_p) %>%
...
for(i in 1:nrow(modules_overview)) {
  ...
  for (feat in rev(feats_to_plot)) {
    ...
    p <- ggplot(tmp, aes_string(x = 'label', y = feat, fill = 'label')) +
      ...
      ylab(get_pretty_names(feat, feat_name_fixes, string_wrap = 40)) +
      theme_bw() +
      theme(legend.position = 'none') +
    ...
    for (j in 1:(length(feats_to_plot)-1)) {
      ...
      if (j>=k) {
        ...
        p <- ggplot(tmp, aes_string(
          ...
          xlab(get_pretty_names(feat, feat_name_fixes, string_wrap = 40)) +
          theme_bw() +
          theme(legend.position = 'none') +
