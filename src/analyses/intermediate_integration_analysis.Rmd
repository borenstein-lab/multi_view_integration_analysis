...
feat_name_fixes <- c(
  ...
  "[M] HILIC.neg_Cluster_0082..nicotinate"="[M] nicotinic acid",
...
Add an indication for each feature whether it is increase/descreased in disease:

```{r}
# sens_analysis_modules2 will be used for plotting later on
sens_analysis_modules2 <- sens_analysis_modules %>%
  mutate(original_feature = feature) %>%
  mutate(feature = ifelse(feature %in% names(feat_name_fixes), feat_name_fixes[feature], feature)) %>%
  mutate(feature = gsub('T__([a-zA-Z]*)\\.([a-zA-Z0-9_]*)$', 'T__\\1_\\2', feature)) %>%
...
detailed_modules %>%
  ...
  left_join(all_utests %>% select(dataset, feature, increased_in), by = c('dataset','feature')) %>%
  rename(Dataset = dataset) %>%
  rename(Module = module) %>%
...
tmp <- bind_rows(lapply(names(diablo_input), function(ds) {
  ds_input = diablo_input[[ds]]
  data.frame(dataset = ds, feature = lapply(ds_input$X, colnames) %>% unlist())
  })) %>%
  mutate(original_feature = feature) %>%
  mutate(feature = gsub('T__([a-zA-Z]*)\\.([a-zA-Z0-9_]*)$', 'T__\\1_\\2', feature)) %>%
  mutate(feature = gsub('_Cluster[0-9]+$', '', feature)) 
...
for(i in 1:nrow(modules_overview)) {
  ...
  edge_cutoff <- gsub('keep_1[0-9]//des_0\\.[0-9]//nrep_10//nfol_5//ncom_5//edge_','',modules_overview$run_id[i]) %>% as.numeric()
  ...
  vertices <- edges %>% 
    ...
    rename(id = `source`) %>%
    distinct() %>%
    mutate(type = substr(id, 1, 1)) %>%
    mutate(color = gplots::col2hex(feature_type_color_map[type])) 
  
  # Get pretty names
  vertices$pretty_name <- 
    paste0('[', vertices$type, '] ',
           get_feature_descriptions(vertices$id))
