...
all_settings_overview <- modules_overview %>%
  ...
  summarise(n_modules = n(), 
            ...
            n_interesting_modules = sum(multi_view & avg_spear_corr > 0.2 & mean_module_auc > 0.7 & (n_feats %in% 5:16)),
...
plot_overall_summary_module_aucs(rf_results, summary_aucs, datasets_A)
```
...
```{r fig.height=6.7, fig.width=5.7}
# Only main datasets
tmp <- plot_module_stats(
    sens_analysis_modules, 
    module_cross_view_corrs, 
    summary_module_aucs %>% 
      left_join(
...
message('Modules were identified for ', 
        n_distinct(module_stats_summary$dataset[module_stats_summary$dataset %in% datasets_A]), 
        ' datasets of ', length(datasets_A), ' analyzed')

message(min(table(module_stats_summary$dataset[module_stats_summary$dataset %in% datasets_A])), '-',
        max(table(module_stats_summary$dataset[module_stats_summary$dataset %in% datasets_A])), 
        ' modules were identified per dataset, average: ',
        round(mean(table(module_stats_summary$dataset[module_stats_summary$dataset %in% datasets_A])),2))

message('Module sizes ranged between ',
        min(module_stats_summary$n_features[module_stats_summary$dataset %in% datasets_A]), ' to ',
        max(module_stats_summary$n_features[module_stats_summary$dataset %in% datasets_A]), 
        ', average: ',
        round(mean(module_stats_summary$n_features[module_stats_summary$dataset %in% datasets_A]), 2))
...
module_stats_summary %>%
  ...
  rename(`Average inter-view correlations` = avg_spear_corr) %>%
...
tmp <- sens_analysis_modules %>%
  ...
  pull(perc_contributors)
message('Examining the XX modules with an AUC > 0.7, we observe that, on average, only ',
        round(mean(tmp),2),'% (SD: ',round(sd(tmp),2),
...
for (i in 1:nrow(module_stats_summary)) {
  ds <- module_stats_summary$dataset[i]
  modu <- module_stats_summary$module[i]
...
for(i in 1:nrow(modules_overview)) {
  ...
  if (is.na(modules_overview$avg_spear_corr[i])) next;
  if (modules_overview$avg_spear_corr[i] < 0.3) next;
  if (modules_overview$mean_module_auc[i] < 0.6) next;
  if (modules_overview$n_feats[i] > 24) next;
...
library(factoextra)
res.pca <- prcomp(tmp %>% select(where(is.numeric)) %>% na.omit(), scale = TRUE)
...
```{r eval=FALSE}
library(ppcor)
# Map correlations between the 3 amino acids: Phe, Val, Leu and top CRC-associated species in Yachida et al.
crc_species <- c(
  'T__Fusobacterium.nucleatum.subsp..nucleatum',
  'T__Fusobacterium.nucleatum.subsp..animalis',
  'T__Fusobacterium.nucleatum.subsp..fusiforme_Cluster257',
  'T__Parvimonas.micra',
  'T__Fusobacterium.nucleatum.subsp..vincentii',
  'T__Fusobacterium.nucleatum.subsp..polymorphum',
  'T__Atopobium.parvulum',
  'T__Actinomyces.odontolyticus',
  'T__Bilophila.wadsworthia',
  'T__Peptostreptococcus.anaerobius',
  'T__Peptostreptococcus.stomatis',
  'T__Gemella.morbillorum',
  'T__Akkermansia.muciniphila',
  'T__Fusobacterium.simiae',
  'T__Fusobacterium.canifelinum',
  'T__Gemella.haemolysans'
  )

other_mm <- c('M__C00123_Leu','M__C00183_Val','M__C00079_Phe')

tmp <- data.frame()
for (mm in c('M__Cysteine.glutathione.disulphide..Divalent')) {
  for (ss in crc_species) {
    mm_vec <- diablo_input$`CRC (Yachida, 2019)`$X$M[,mm]
    ss_vec <- diablo_input$`CRC (Yachida, 2019)`$X$`T`[,ss]
    spear <- cor.test(mm_vec, ss_vec, method = 'spearman')
    pspear <- pcor.test(mm_vec, ss_vec, z = as.numeric(diablo_input$`CRC (Yachida, 2019)`$Y == 'disease'), method = 'spearman')
    tmp <- bind_rows(
      tmp,
      data.frame(
        metab = mm,
        species = ss,
        spear_rho = spear$estimate,
        spear_p = spear$p.value,
        pspear_rho = pspear$estimate,
        pspear_p = pspear$p.value
      )
    )
  }
  ...
  for (ss in other_mm) {
    mm_vec <- diablo_input$`CRC (Yachida, 2019)`$X$M[,mm]
    ss_vec <- diablo_input$`CRC (Yachida, 2019)`$X$M[,ss]
    spear <- cor.test(mm_vec, ss_vec, method = 'spearman')
    pspear <- pcor.test(mm_vec, ss_vec, z = as.numeric(diablo_input$`CRC (Yachida, 2019)`$Y == 'disease'), method = 'spearman')
    tmp <- bind_rows(
      tmp,
