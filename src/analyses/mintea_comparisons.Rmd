...
p1 +
  ...
  annotate("text", lineheight = .92, x = 0.47, y = 17.5, angle = 45, label = "Higher false\ndiscovery rate", size = 4) +
  geom_segment(aes(x = 0.1,y = 9,xend = 1.33,yend = 22),
...
```{r fig.width=9, fig.height=9.2}
leg <- get_legend(p6)
main <- plot_grid( p5 + theme(legend.position = 'none'),
          p6 + theme(legend.position = 'none'), 
          p2 + theme(legend.position = 'none'),
          p3 + theme(legend.position = 'none'),
          p1 + theme(legend.position = 'none'),
          #p4 + theme(legend.position = 'none'),
          nrow = 3, align = "hv", axis = 'tlrb') #, labels = 'AUTO', label_y = 1.03, label_size = 15)
