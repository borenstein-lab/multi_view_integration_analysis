...
p2 <- ggplot(tmp %>% filter(keepX_category == 'Avg. component size: ~20'),
             aes(x = perc_samples_to_exclude2, y = perc_shared_feats_mean/100, group = method, fill = method)) +
  geom_col(color = 'black', position = position_dodge(.8), alpha = 0.7, width = .7) +
  geom_errorbar(aes(ymin = perc_shared_feats_mean/100 - perc_shared_feats_sd/100,
                    ymax = perc_shared_feats_mean/100 + perc_shared_feats_sd/100),
                width = 0.3, position=position_dodge(.8)) +
  scale_fill_manual(values = method_colors, name = 'Method') +
  theme_bw() +
  ylab("Percent of features\nshared between models") +
