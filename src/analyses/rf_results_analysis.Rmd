...
Change to "pretty" dataset names and narrow to "main" datasets only.
...
cv_datasets_summary <- cv_datasets_summary %>% filter(dataset %in% datasets_A)
cv_results <- cv_results %>% filter(dataset %in% datasets_A)
feat_imp <- feat_imp %>% filter(dataset %in% datasets_A)
feat_imp_sum <- feat_imp_sum %>% filter(dataset %in% datasets_A)
...
cv_datasets_summary %>%
  left_join(
    cv_results %>%
      group_by(dataset, shuffled) %>%
      summarise(avg_feat_final = round(mean(n_features_for_train_final),3),
                sd_auc = round(sd(out_of_fold_test_auc, na.rm=T),3), 
                .groups = 'drop'),
    by = c('dataset','shuffled')
  ) %>%
  filter(!shuffled) %>%
  select(dataset, feature_set_type, fs_type, 
         n_features_origin_T, n_features_origin_P, n_features_origin_M, n_features_origin_S,
         avg_feat_final, mean_out_of_fold_test_auc, sd_auc) %>%
  ...
  rename(`Average number of features selected for final model` = avg_feat_final) %>%
  rename(`Cross-validated average AUC` = mean_out_of_fold_test_auc) %>%
  rename(`Cross-validated SD AUC` = sd_auc) %>%
...
```{r, fig.height=7, fig.width=3.5, warning = FALSE}
plot_basic_stats2(
...
```{r, fig.height=5.2, fig.width=5, warning = FALSE}
# plot_basic_stats(
#   cv_results = cv_results, 
#   feature_type_color_map = feature_type_color_map
#   )
